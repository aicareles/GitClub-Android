...
public class SPUtils {
     ...
     * @param device
     */
   /* public static void putBleAvertiseDevice(Context context, String key, BleDevice device) {
        if (device == null)return;
        SharedPreferences sp = context.getSharedPreferences(FILE_NAME, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sp.edit();
        String deviceStr = JSON.toJSONString(device);
        editor.putString(key, deviceStr);
        SharedPreferencesCompat.apply(editor);
    }

    public static BleDevice getBleAvertiseDevice(Context context, String key){
        SharedPreferences sp = context.getSharedPreferences(FILE_NAME, Context.MODE_PRIVATE);
        String deviceStr = sp.getString(key, null);
        Log.e("getBleAvertiseDevice", "getBleAvertiseDevice: "+deviceStr);
        if(!TextUtils.isEmpty(deviceStr)){
            return JSON.parseObject(deviceStr, BleDevice.class);
        }
        return null;
    }*/
